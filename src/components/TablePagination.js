...
function optionByValue(options, value, valueKey = 'value') {
 ...
 * Same API / layout as CLIENT `TablePagination`.
 *
 * Props:
 ...
 * @param {number} [defaultRows=20] — fallback limit if `limit` is invalid
 ...
 * @param {boolean} [isLastPage] — when set, disables Next when true
...
const TablePagination = ({
    ...
    return (
        <div
            className={`border-t border-slate-200 dark:border-gray-700 bg-gradient-to-r from-slate-50 to-white dark:from-gray-900 dark:to-gray-900 px-4 py-3 sm:py-3.5 sm:px-5 ${className}`.trim()}
        >
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                {(showRange || showRows) && (
                    <div className="flex min-w-0 flex-wrap items-center gap-3">
                        {showRange && (
                            <div className="whitespace-nowrap text-sm text-slate-500 dark:text-gray-400">
                                Showing{' '}
                                <span className="font-medium text-slate-700 dark:text-gray-200">{rangeStart}</span>
                                {' to '}
                                <span className="font-medium text-slate-700 dark:text-gray-200">{rangeEnd}</span>
                                {' of '}
                                <span className="font-medium text-slate-700 dark:text-gray-200">{safeTotal}</span>
                            </div>
                        )}
                        {showRows && onLimitChange && (
                            <div className="flex items-center gap-2">
                                <label htmlFor={rowsFieldId} className="text-xs font-medium text-slate-500 dark:text-gray-400">
                                    Rows per page
                                </label>
                ...
                {showControls && (
                    <div className="flex min-w-0 w-full flex-wrap items-center justify-end gap-2 sm:w-auto sm:flex-nowrap sm:gap-3">
                        <div className="flex flex-wrap items-center justify-end gap-1 sm:gap-1.5">
                                ...
                                <button
                                    ...
                                    className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm transition-colors ${
                                        safePage === 1
                                            ...
                                            : 'text-slate-700 dark:text-gray-200 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600'
                                    }`}
                                >
                            ...
                            <span className="inline-flex h-9 min-w-[2.25rem] items-center justify-center rounded-lg border border-slate-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-2 text-sm font-semibold tabular-nums text-slate-800 dark:text-gray-100">
                                {safePage}
                            </span>
                            <button
                                type="button"
                        ...
                        {showJumpBlock && (
                            <>
                                <span className="hidden h-6 w-px shrink-0 self-center bg-slate-200 dark:bg-gray-600 sm:block" aria-hidden />
                                <form
                                    onSubmit={handleJumpSubmit}
                                    className="flex min-w-0 shrink-0 items-center gap-2"
                                >
                                    <span className="hidden text-xs text-slate-500 dark:text-gray-400 lg:inline">Jump</span>
                                    <input
                                        type="text"
                                        inputMode="numeric"
                                        autoComplete="off"
                                        ...
                                        className={`w-16 rounded-lg border px-2 py-2 text-center text-sm focus:outline-none focus:ring-2 ${
                                            jumpPageInputInvalid
                                                ...
                                                : 'border-slate-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-slate-800 dark:text-gray-100 focus:border-indigo-500 focus:ring-indigo-500/30'
                                        }`}
                                    />
                                    <button
                                        type="submit"
                                    ...
                                    <span className="text-xs tabular-nums text-slate-500 dark:text-gray-400">/ {totalPagesForJump}</span>
                                </form>
                            </>
                        )}
                    </div>
