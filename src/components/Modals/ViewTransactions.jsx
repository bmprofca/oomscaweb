...
const CommonParticulars = ({ transaction }) => {
    ...
    if (particular.details && typeof particular.details === 'object') {
        const contactKeys = new Set(['email', 'mobile', 'country_code', 'phone', 'address']);
        const entries = Object.entries(particular.details).filter(
            ([key, val]) => val != null && val !== '' && !contactKeys.has(key)
        );
        if (entries.length === 0) {
            return <p className="text-slate-600 text-sm">{particular.details?.name || '—'}</p>;
        }
        return (
            ...
            <SectionCard title="Party" icon={FiUsers} iconClass="text-blue-500">
                {partyDetails ? (
                    <>
                        ...
                        {!partyDetails.bank && !partyDetails.account_no && !partyDetails.ifsc && !partyDetails.branch ? (
